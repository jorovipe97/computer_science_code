	...
	try:
		with open(fileName, 'r') as fp:
			...
			for line in fp:
				...
				if p1.ignoreLine or parser.isMultilineComment:
					continue;
				commandType = p1.commandType();
				'''
				# Codigo para manejar los comentario multilinea
				if line.find('/*') != -1:
					multiLineComment = True;

				# Si existe un terminador de multiline comment
				if line.find('*/') != -1:
					multiLineComment = False;
		...
		with open(asmWithoutSymbolsName, 'r') as fp:
			...
			for line in fp:
				...
				if commandType == 'A_COMMAND':    
					...
					outfile.write('101' + translator.erase(p.erase()).zfill(13) + '\n');
					pass
				else:
